    ...
    // ----------------------------------------------------------------------------
    package org.opengts.servers.concox;
    ...
    //            return ServerSocketThread.PACKET_LEN_ASCII_LINE_TERMINATOR; // <-- change this for binary packets
            //int llo=(packet[2]+0x05);
    //	    int casta = (int)packet[2] & 0xff;
            
            byte a=5;
    //	    byte c = (byte) (a+packet[2]);
    byte c=0;
    Print.logInfo("body_status: " + packet[2]);
    if(packet[2]>0){
    c=(byte)(packet[2]+a);
    }
            return (int)c==0? ServerSocketThread.PACKET_LEN_END_OF_STREAM:(int)c;
        ...
        public byte[] getHandlePacket(byte pktBytes[]) 
        {
            ...
            if ((pktBytes != null) && (pktBytes.length > 0)) {
                ...
                Print.logInfo("GT06 Recv[HEX]: " + StringTools.toHexString(pktBytes));
    ...
    //                int fld_1 = (int)p.readLong(2,0L);   // parse 0x010203   into 'fld_1'
    //                Print.logInfo("1: " + StringTools.toHexString(fld_1));
    //                int fld_2 = (int)p.readLong(1,0L);   // parse 0x010203   into 'fld_1'
    //                Print.logInfo("2: " + StringTools.toHexString(fld_2));
    //                int fld_3 = (int)p.readLong(2,0L);   // parse 0x010203   into 'fld_1'
    //                Print.logInfo("3: " + StringTools.toHexString(fld_3));

    //                Print.logInfo("xxx: " + StringTools.toHexString(pktBytes[0]));
    ...
    //                  int fld_4 = (int)p.readLong(2,0L);   // parse 0x0A0B     into 'fld_2'
                ...
                Print.logInfo("GT06 Empty packet received ...");
    ...
    //Handing incoming packets-  gt06n

    // will be using a white list as the gps terminal will abruptly send undocumented packets
    // these particular packets will block the incoming data packets and needs a null return 
    // each time, as the erroneous data packet differ from a terminal to another i white listed only the usefull packets to our needs

    Print.logInfo("head: " + StringTools.toHexString(b[0])+StringTools.toHexString(b[1]));
    Print.logInfo("length: " + StringTools.toHexString(b[2]));//25h  37
    Print.logInfo("protocol: " + StringTools.toHexString(b[3]));
    ...
    switch(b[3]){
        case 0x13:
            Print.logInfo("13: ");
            Print.logInfo("Device info: " + StringTools.toHexString(b[4]));
            Print.logInfo("Voltage degree: " + StringTools.toHexString(b[5]));
            Print.logInfo("GSM Signal: " + StringTools.toHexString(b[6]));
            break;
        ...
        case 0x12:
            Print.logInfo("12: ");
            Print.logInfo("Date: " + StringTools.toHexString(b[4]) + StringTools.toHexString(b[5]) + StringTools.toHexString(b[6]) + StringTools.toHexString(b[7]) + StringTools.toHexString(b[8]) + StringTools.toHexString(b[9]));
            Print.logInfo("GPS info: " + StringTools.toHexString(b[10]));
            Print.logInfo("Latitude: " + StringTools.toHexString(b[11]) + StringTools.toHexString(b[12]) + StringTools.toHexString(b[13]) + StringTools.toHexString(b[14]));
            Print.logInfo("Longitude: " + StringTools.toHexString(b[15]) + StringTools.toHexString(b[16]) + StringTools.toHexString(b[17]) + StringTools.toHexString(b[18]));
            Print.logInfo("Speed: " + StringTools.toHexString(b[19]));
            Print.logInfo("Status/Course: " + StringTools.toHexString(b[20]) + "/" + StringTools.toHexString(b[21]));
        
            //lat
            int lati = ((b[11] & 0xFF) << 24) + ((b[12] & 0xFF) << 16) + ((b[13] & 0xFF) << 8) + (b[14] & 0xFF);
            ...
            //long
            int longi = ((b[15] & 0xFF) << 24) + ((b[16] & 0xFF) << 16) + ((b[17] & 0xFF) << 8) + (b[18] & 0xFF);
            ...
            //speed
            double   speedKPH   = StringTools.parseDouble(b[19],0.0);
            ...
            //int com=(h1&0xFF)<<8;
            //com+=(h2&0xFF);
            //double   heading  = (double)com;
            //double   heading   = StringTools.parseDouble(b[20],0.0);
            ...
            Print.logInfo("devs: " + devs);
        
        
                    GPSEvent gpsEvent = new GPSEvent(Main.getServerConfig(),this.ipAddress, this.clientPort, devs);
                    Device device = gpsEvent.getDevice();
                    if (device == null) {
                        return null;
                    }
        ...
        case 0x01:
            Print.logInfo("01: ");
            Print.logInfo("id: " + StringTools.toHexString(b[4]) + StringTools.toHexString(b[5])+StringTools.toHexString(b[6])+StringTools.toHexString(b[7])+StringTools.toHexString(b[8])+StringTools.toHexString(b[9])+StringTools.toHexString(b[10])+StringTools.toHexString(b[11]));
            this.devs=(StringTools.toHexString(b[4]) + StringTools.toHexString(b[5])+StringTools.toHexString(b[6])+StringTools.toHexString(b[7])+StringTools.toHexString(b[8])+StringTools.toHexString(b[9])+StringTools.toHexString(b[10])+StringTools.toHexString(b[11]));
            break;

        default: 
            Print.logError("undocumented packet: " + StringTools.toHexString(b));
            return null;
    ...
    // the data packet with protocol number 8A is an unregistered packet sent by the terminal
    // this packet needs to be ignored and close socket in order for the device to send proper data packet
    // important note this packets are not referenced in the Concox GPS gt06n documentation

    // if(b[3]==0x8A){
    //     Print.logError("Unregistered Packet: " + StringTools.toHexString(b[3]));
    //     return null;
    // }

    // // the data packet with protocol number 20 is also an unregistered packet sent by the terminal
    // // this packet also requires ignoring and socket closing otherwise it will interfere with proper gps data sending
    // // important note this packets are not referenced in the Concox GPS gt06n documentation

    // if(b[3]==0x20){
    //     Print.logError("Information Packet: " + StringTools.toHexString(b[3]));
    //     return null;
    // }
    ...
    Print.logInfo("serial: " + StringTools.toHexString(b[b[2]-6+5])+StringTools.toHexString(b[b[2]-5+5]));
    Print.logInfo("crc: " + StringTools.toHexString(b[b[2]-4+5])+StringTools.toHexString(b[b[2]-3+5]));
    Print.logInfo("stop: " + StringTools.toHexString(b[b[2]-2+5])+StringTools.toHexString(b[b[2]-1+5]));
    ...
    //}
        ...
        private byte[] parseInsertRecord_ASCII_11(byte b[])
        {
    ...
    //int i = ((0x06 & 0xFF) << 24) + ((0x15 & 0xFF) << 16) + ((0xdc & 0xFF) << 8) + (0x7a & 0xFF);
    //double d = (double)i/30000;
    //int i2 = (int)d/60;
    //double d2 = d - i2*60;
    //int d3 = (int)((d - i2*60)*1000000);
    //String aString = Integer.toString(i2) + "." +Integer.toString(d3);
    //Print.logInfo("lat: " + i + " " + d + " " + i2 + " " + d2 + " " + aString);
    //Print.logInfo("lat: " + aString);
    ...
    */
    ...
    double latd = (double)lati/30000;
    int lati2 = (int)latd/60;
    double latd2 = latd - lati2*60;
    int latd3 = (int)((latd - lati2*60)*1000000);
    String latString = Integer.toString(lati2) + "." +Integer.toString(latd3);
    //String latString = Integer.toString(lati2) + Integer.toString(latd3);
    Print.logInfo("lat: " + latString);
    double   latitude   = StringTools.parseDouble(latString,0.0);
    ...
    double longd = (double)longi/30000;
    int longi2 = (int)longd/60;
    double longd2 = longd - longi2*60;
    int longd3 = (int)((longd - longi2*60)*1000000);
    String longString = Integer.toString(longi2) + "." +Integer.toString(longd3);
    Print.logInfo("long: " + longString);
    double   longitude  = StringTools.parseDouble(longString,0.0);
    */
    ...
    //        }
        ...
        //   java -cp <classpath> org.opengts.servers.gt06.TrackClientPacketHandler {options}
            ...
            if (RTConfig.hasProperty(Main.ARG_PARSEFILE)) {
                ...
                try {
                    ...
                    for (;;) {
                        ...
                        Print.logInfo("GT06 data read: " + data);
